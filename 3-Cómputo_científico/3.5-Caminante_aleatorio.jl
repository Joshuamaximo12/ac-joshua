...
# Generar una muestra de números aleatorios distribuidos uniformemente en [a,b]
A = 0
B = 10
muestra = [aleatoriouniforme(A, B) for _ in 1:10000]

# Crear un histograma de la muestra
histogram(muestra, bins = 50, legend = false, xlabel = "Valor", ylabel = "Frecuencia", title = "Distribución Uniforme")
end
...
#function rW(nnn::Int)
    ...
    #current_position = 0
    #for i in 1:nnn
       # sStep = rand([-1, 1])  # Randomly choose either -1 or 1
       # current_position += sStep
        #push!(posicions, current_position)
    #end
    #return posicions
#end

# Function to plot position versus number of steps
#function wp(posicions)
 #   n_steps = length(posicions) - 1
  #  sSteps = 0:n_sSteps
   # plot(sSteps, posicions, seriestype = :scatter, xlabel = "Number of Steps", ylabel = "Position")
#end

# Function to animate the random walk
#function animaCaminata(posicions)
 #   n_steps = length(posicions) - 1
  #  sSteps = 0:n_sSteps
   # anim = Animation()
    #for i in 1:length(posicions)
     #   pP = plot(sSteps[1:i], posicions[1:i], seriestype = :scatter, xlabel = "Number of Steps", ylabel = "Position", legend = false)
      #  frame(anim, pP)
    #end
    #return anim
#end
...
# Test the functions
#nN2 = 50  # Number of steps for random walk
#posicions = rW(nN2)
#wp(posicions)

# Uncomment the following line to animate the random walk
#anim = animaCaminata(posicions)
#gif(anim, "random_walk_animation.gif", fps = 10)
#end
...
# Damos valores para a y b
a = 1
b = 10
numero_aleatorio = aleatorioUniforme(a, b)
println(numero_aleatorio)
end

#La función aleatorioUniforme toma dos argumentos, a y b, que representan los extremos del intervalo [a,b]. Luego, utiliza la función rand() de Julia para generar un número aleatorio distribuido uniformemente en el intervalo [a,b]. Luego se realiza una transformación lineal para ajustar el rango del número aleatorio al intervalo [a,b] que es equivalente a tomar el valor de "a" y se le suma la diferencia de "b" menos "a", y se multiplica por un número aleatorio.
#Cabe mencionar que este número aleatorio vive en el intervalo (a,b), y por cada iteración dada, el número cambia.

# ╔═╡ d0951f5d-7d24-423c-8b55-34cf7a88210f
...
begin
function unPaso()  #la función 'un paso no toma valores de entrada'
    if rand(Bool)  #se utiliza como sugerencia la función 'rand(Bool)'
        return 1
    else
        return -1
    end
...
# Lo que hace la función rand(Bool) es generar un valor aleatorio booleano (true o false) con probabilidad igual de ocurrencia. Luego, dentro de la función unPaso, se utiliza una declaración if para devolver 1 si el resultado de rand(Bool) es true y -1 si es false.

# ╔═╡ 88b1addb-19b1-417f-b372-e84bca6d6d26
begin
resultado = unPaso()
println(resultado)
end
#Es cierto que tiene la misma probabilidad que sea 1 o -1, sin embargo, no depende de las iteraciones realizadas, porque a veces con solo una iteración cambia el valor, pero en otras, con más de una iteración sigue siendo el mismo valor.
...
begin
	function variosPasos(N) #Esta función toma como argumento la cantidad de pasos que se le asigne
    return rand([-1, 1], N)
end

# Ejemplo
N = 5
result = variosPasos(N)
println(result)
end
...
begin
function caminataAleatoria(nn)
    caminata = [0]  # Inicializar la caminata con la posición inicial en 0
    
    for i in 1:nn
        # Generar un número aleatorio -1 o 1 para determinar el siguiente paso, podemos usar como base, el ejemplo anterior para obtner calores aleatorios entre [-1, 1]
        paso = rand([-1, 1])
        posicion_actual = caminata[end]  # Obtener la posición actual
        nueva_posicion = posicion_actual + paso  # Calcular la nueva posición
        
        push!(caminata, nueva_posicion)  # Agregar la nueva posición al arreglo de la caminata
    end
...
# Ejemplo de uso
pasos = 10
caminata = caminataAleatoria(pasos)
println(caminata)
end

#Aquí se utilizó un bucle for para iterar n veces y generar cada paso de la caminata aleatoria. En cada iteración, se genera un número aleatorio (-1 o 1) para determinar si el paso será hacia la izquierda o hacia la derecha. Luego, se calcula la nueva posición sumando el paso al valor de la posición actual. Finalmente, se agrega la nueva posición al arreglo caminata utilizando la función push!. El resultado se devuelve como un arreglo con n+1 posiciones, incluyendo la posición inicial en 0.

#En el ejemplo de uso, se especifica pasos = 10 para generar una caminata aleatoria de 10 pasos. El resultado se almacena en el arreglo caminata.
...
#Para evitar la sobrescritura de las funciones y variables, solo se le harán pequeñas modificaciones, pero hacen referencia a las mismas utilizadas anterioremente.
begin
...
function CaminataAleatoria(NN)
    ...
    for _ in 1:NN
        ...
        push!(Caminata, Posicion)
    end
    return Caminata
end

n2 = 50  # Número de pasos para la caminata aleatoria

Caminata = CaminataAleatoria(n2)
GraficaCaminata(Caminata)
end

#La función GraficaCaminata toma un arreglo caminata que representa los pasos de una caminata aleatoria en una dimensión y traza la gráfica de posición contra número de pasos.
#La función CaminataAleatoria genera una caminata aleatoria simulando n pasos. Comienza en la posición 0 y en cada paso, se mueve hacia la derecha (+1) o hacia la izquierda (-1) con una probabilidad del 50% cada una. Los pasos y las posiciones se almacenan en un arreglo, que se devuelve al final de la función.
#En el ejemplo proporcionado, se define n2 como 100 y se llama a CaminataAleatoria(n2) para generar una caminata aleatoria de 100 pasos. Luego, se pasa esta caminata como argumento a la función GraficaCaminata para trazar la gráfica correspondiente.
...
# ╔═╡ 02808fdb-3052-4931-b2e1-8f025a063a7b
begin
	function walkplot(walk::AbstractArray)
    steps = 0:length(walk)-1
    plot(steps, walk, seriestype = :scatter, xlabel = "Number of Steps", ylabel = "Position")
end
end

# ╔═╡ 170ffbc7-93a2-42d0-8c7e-7085aa0a89ea
begin
function randomWalk(n::Int)
    walk = [0]
    for i in 1:n
        step = rand([-1, 1])
        position = walk[end] + step
        push!(walk, position)
    end
    return walk
end
...
# ╔═╡ a9096006-dd21-4ba5-bbe5-455d26a61f66
begin
function graphWalk!(walk::AbstractArray)
    steps = 0:length(walk)-1
    plot!(steps, walk, seriestype = :line, linestyle = :dash)
end
end

# ╔═╡ f357c3d9-1773-4ee6-84e7-0d8f45cf4bca
begin
	N2 = 50  # Number of steps for each random walk
walks = [randomWalk(N2) for _ in 1:5]
...
# ╔═╡ 2e5ad9b4-abd0-4635-9505-a1a07342f487
begin
function AleatorioUniformE(a, b)
    return a + (b - a) * rand()
end
...
function caminataAleatoria2D(n)
    ...
    for i in 1:n
        ...
        nueva_pos_h = posiciones_h[end] + paso_h
        nueva_pos_v = posiciones_v[end] + paso_v
        push!(posiciones_h, nueva_pos_h)
        push!(posiciones_v, nueva_pos_v)
    end

    return [posiciones_h, posiciones_v]
end

caminat4 = caminataAleatoria2D(50)

end

#La función caminataAleatoria2D(n) toma un número de pasos n como entrada y simula una caminata aleatoria en dos dimensiones. Crea dos arreglos posiciones_h y posiciones_v para almacenar las posiciones horizontales y verticales respectivamente. Luego, en cada paso, genera dos números aleatorios en el intervalo [-1, 1] utilizando la función AleatorioUniformE(-1, 1). Estos números representan los pasos horizontales y verticales. A continuación, se actualizan las nuevas posiciones sumando los pasos generados a las posiciones anteriores. Finalmente, se agrega la nueva posición a los arreglos posiciones_h y posiciones_v. Al finalizar, se devuelve un arreglo con los dos subarreglos que contienen las posiciones horizontales y verticales respectivamente.
#Luego se llama a la función caminataAleatoria2D(n) con el número de pasos deseado para obtener una simulación de una caminata aleatoria en dos dimensiones.
...
function AleatorioUniformee(a, b)
    return a + (b - a) * rand()
end

function CaminataAleatoria2D(n)
    x_posiciones = zeros(n+1)
    y_posiciones = zeros(n+1)
    
    for i in 2:n+1
        x_posiciones[i] = x_posiciones[i-1] + AleatorioUniformee(-1, 1)
        y_posiciones[i] = y_posiciones[i-1] + AleatorioUniformee(-1, 1)
    end
    
    return [x_posiciones, y_posiciones]
end

function graficaCaminata2D(posiciones)
    plot(posiciones[1], posiciones[2], linestyle=:dash, marker=:circle, markersize=2,
         xlabel="Posición Horizontal", ylabel="Posición Vertical", legend=false)
end
...
# La función graficaCaminata2D generará un gráfica que muestra la trayectoria de la caminata aleatoria como una línea discontinua. 
...
function GraficaCaminata2D(camin4ta)
    plot(camin4ta[1], camin4ta[2], linestyle=:dot, marker=:circle, markersize=2, aspect_ratio=:equal,
        xlabel="Posición horizontal", ylabel="Posición vertical", title="Caminata aleatoria 2D")
end

N3 = 20  # Número de pasos
camin4ta = Caminataaleatoria2D(N3)
GraficaCaminata2D(camin4ta)

	function graficaCaminata2D!(camin4tas)
    plot(xlabel="Posición horizontal", ylabel="Posición vertical", title="Caminatas aleatorias 2D")
    
    for camin4ta in camin4tas
        plot!(camin4ta[1], camin4ta[2], linestyle=:dot, marker=:circle, markersize=2, aspect_ratio=:equal)
    end
...
for i in 1:num_caminatas
    camin4ta = Caminataaleatoria2D(N3)
    push!(camin4tas, camin4ta)
end

graficaCaminata2D!(camin4tas)
...
function caminatAAleatoria2D(n)
    ...
    y = zeros(n+1)
    for i in 2:(n+1)
        dx = aleatoriOUniforme(-1, 1)
        dy = aleatoriOUniforme(-1, 1)
        x[i] = x[i-1] + dx
        y[i] = y[i-1] + dy
    end
    return [x, y]
...
function animaCaminata2D(caminataaa)
    len = length(caminataaa[1])
    anim = @animate for i in 1:len
        GraficacaminatA2D(caminataaa[1][1:i], caminataaa[2][1:i])
    end
    gif(anim, "caminata.gif", fps = 10)
end

function GraficacaminatA2D(x, y)
    plot(x, y, marker = :circle, markersize = 2, color = :blue, legend = false, xlims = (-maximum(abs.(x)), maximum(abs.(x))), ylims = (-maximum(abs.(y)), maximum(abs.(y))), aspect_ratio = :equal)
end

# Ejemplo de uso
caminataaa = caminatAAleatoria2D(20)
animaCaminata2D(caminataaa)
end
...
function caminata_aleatoria_3d(num_pasos)
    ...
    for i in 1:num_pasos
        # Generar un desplazamiento aleatorio en cada dirección
        dXx = rand([-1, 1])
        dYy = rand([-1, 1])
        dZz = rand([-1, 1])
        ...
        # Agregar las nuevas coordenadas a las listas
        push!(Xx, new_Xx)
        push!(Yy, new_Yy)
        push!(Zz, new_Zz)
    end
...
# Configurar el número de pasos de la caminata
num_Pasos = 20

# Realizar la caminata aleatoria
Xx, Yy, Zz = caminata_aleatoria_3d(num_Pasos)

# Graficar la trayectoria
plot3d(Xx, Yy, Zz, line = (:path, 2), marker = (:sphere, 4))
xlabel!("X")
ylabel!("Y")
zlabel!("Z")
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "6f5a708bd699cde31d9a0f32ccb5ea0c6ca882d6"
